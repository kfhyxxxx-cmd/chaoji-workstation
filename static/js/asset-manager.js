...
function setStatus(text='准备就绪'){ if(statusEl){ statusEl.textContent = text || '准备就绪'; statusEl.title = text || '准备就绪'; statusEl.classList.remove('expanded'); } }
        ...
        ? `<div class="classification-chips">${chips.map(chip => {
            const filterAttr = kind === 'localup'
                ...
                : kind === 'asset'
                    ...
                    : '';
            const delAttr = kind === 'localup'
                ? ` data-localup-class-delete="${escapeAttr(itemId)}" data-localup-class-delkey="${escapeAttr(chip.key)}"`
                : kind === 'asset'
                    ? ` data-asset-class-delete="${escapeAttr(itemId)}" data-asset-class-delkey="${escapeAttr(chip.key)}"${libAttr}`
                    : '';
            return `<button class="${activeKey === chip.key ? 'active' : ''}" type="button"${filterAttr} title="${escapeAttr(chip.label)}"><span>${escapeHtml(chip.tag)}</span><span class="class-chip-del"${delAttr} title="删除此标签">×</span></button>`;
        }).join('')}</div>`
        : '';
}
...
async function deleteClassificationTag(itemId, delKey, kind, libraryId=''){
    const item = kind === 'localup' ? findLocalUpload(itemId) : findAssetItem(itemId);
    if(!item) return;
    const flat = Array.isArray(item?.classification?.flat) ? [...item.classification.flat] : [];
    const parsed = parseAssetClassFilterKey(delKey);
    if(!parsed) return;
    const newFlat = flat.filter(entry => String(entry?.dimension || '') !== parsed.dimension || String(entry?.tag || '') !== parsed.tag);
    if(newFlat.length === flat.length) return;
    const newClassification = {...(item.classification || {}), flat: newFlat};
    if(kind === 'localup'){
        try {
            const data = await apiJson('/api/local-assets/classification', {
                method:'PATCH',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({name:item.file || item.id, classification:newClassification})
            });
            if(data.item) Object.assign(item, data.item);
            setStatus('已删除标签');
        } catch(err) {
            setStatus(err.message || '删除标签失败');
        }
    } else {
        try {
            const data = await apiJson(`/api/asset-library/items/${encodeURIComponent(itemId)}`, {
                method:'PATCH',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({classification:newClassification, library_id:libraryId})
            });
            assetLibrary = data.library || assetLibrary;
            setStatus('已删除标签');
        } catch(err) {
...
async function handleClick(event){
    ...
    const assetClassDel = target.closest?.('[data-asset-class-delete]');
    if(assetClassDel){ event.stopPropagation(); await deleteClassificationTag(assetClassDel.dataset.assetClassDelete || '', assetClassDel.dataset.assetClassDelkey || '', 'asset', assetClassDel.dataset.assetClassLib || ''); return; }
    const localupClassDel = target.closest?.('[data-localup-class-delete]');
    if(localupClassDel){ event.stopPropagation(); await deleteClassificationTag(localupClassDel.dataset.localupClassDelete || '', localupClassDel.dataset.localupClassDelkey || '', 'localup'); return; }
...
statusEl?.addEventListener('click', () => { statusEl.classList.toggle('expanded'); });
