...
function renderVideoTrustedAssetControl(){
    ...
    const hasManualUrls = src === 'manual' && currentSmartMediaLinks().length > 0;
    html += `<div class="trusted-source-row">
        ...
        <button type="button" class="smart-pill trusted-src-pill ${src === 'manual' ? 'active' : ''}" data-trusted-source="manual" title="手动输入媒体 URL 或 asset:// 地址"><i data-lucide="link"></i><span>输入网址${hasManualUrls ? ' ✓' : ''}</span></button>
        ${hasManualUrls ? `<button type="button" class="smart-pill trusted-src-pill clear" data-clear-manual-urls title="清除手动输入的网址"><i data-lucide="x"></i><span>清除</span></button>` : ''}
    </div>`;
    return html;
}
...
async function setCurrentSmartManualVideoUrl(){
    ...
    // 自动补全 asset:// 前缀：输入 asset-xxx 等同于 asset://asset-xxx
    const normalizedUrls = urls.map(url => {
        const trimmed = String(url || '').trim();
        if(!trimmed) return '';
        if(/^asset:\/\//i.test(trimmed)) return trimmed;
        if(/^asset-/i.test(trimmed)) return 'asset://' + trimmed;
        return trimmed;
    });
    const invalid = normalizedUrls.find(url => url && !isRemoteVideoReferenceUrl(url));
    if(invalid){
...
function bindDynamicParams(){
    ...
    dynamicParams.querySelectorAll('[data-clear-manual-urls]').forEach(btn => {
        btn.onclick = event => {
            event.preventDefault();
            event.stopPropagation();
            clearManualSmartVideoUrl();
...
function renderInputThumbsRow(node){
    ...
    const thumbsHtml = dedup.map((img, i) => {
        ...
        const hasAssetUri = !!((settings.videoManualAssetUris || {})[sourceUrl]);
        const assetBtn = kind === 'image' ? `<button class="input-thumb-asset ${hasAssetUri ? 'active' : ''}" type="button" data-asset-uri-bind="${escapeHtml(sourceUrl)}" title="${hasAssetUri ? '已绑定 asset:// 认证地址（点击修改）' : '绑定 asset:// 认证地址'}">🔗</button>` : '';
        return `<div class="input-thumb ${isSelf ? 'input-self' : ''} ${removable ? 'input-manual-ref' : ''} ${hasAssetUri ? 'input-asset-bound' : ''}" draggable="false" data-thumb-index="${i}" data-node-id="${escapeHtml(img.nodeId || '')}" data-image-index="${img.imageIndex ?? ''}" data-url="${escapeHtml(img.url || '')}" data-source-url="${escapeHtml(sourceUrl)}" title="${escapeHtml(`${img.name || tr('smart.inputNum').replace('{n}', String(i + 1))} · ${title}`)}">${inner}<span class="input-thumb-label">${escapeHtml(label)}</span>${assetBtn}${removeBtn}</div>`;
    }).join('');
...
function bindInputThumbAssetActions(){
    inputThumbsRow?.querySelectorAll('[data-asset-uri-bind]').forEach(btn => {
        btn.onclick = async event => {
            event.preventDefault();
            event.stopPropagation();
            const sourceUrl = btn.dataset.assetUriBind || '';
            const currentUri = ((settings.videoManualAssetUris || {})[sourceUrl]) || '';
            if(currentUri){
                // 已绑定：先问是否清除
                const action = confirm(`当前已绑定：\n${currentUri}\n\n点击「确定」清除绑定\n点击「取消」修改地址`);
                if(action){
                    // 确定 → 清除
                    const uris = {...(settings.videoManualAssetUris || {})};
                    delete uris[sourceUrl];
                    settings.videoManualAssetUris = Object.keys(uris).length ? uris : undefined;
                    toast('已清除 asset:// 绑定');
                    persistActiveSmartSettings();
            ...
            const value = prompt(currentUri ? '修改 asset:// 认证地址（留空清除）：' : '输入 asset:// 认证地址（在火山方舟控制台获取）：', currentUri);
            if(value === null) return;
            ...
            if(!trimmed){
                ...
                // 自动补全 asset:// 前缀：输入 asset-xxx 等同于 asset://asset-xxx
                let finalUri = trimmed;
                if(!/^asset:\/\//i.test(finalUri)){
                    if(/^asset-/i.test(finalUri)){
                        finalUri = 'asset://' + finalUri;
                    } else {
                        toast('请输入以 asset:// 或 asset- 开头的认证地址');
                        return;
                    }
                }
                settings.videoManualAssetUris = {...(settings.videoManualAssetUris || {}), [sourceUrl]: finalUri};
                toast('已绑定 asset:// 认证地址');
            }
            persistActiveSmartSettings();
            if(inputThumbsRow) delete inputThumbsRow.dataset.thumbsSig;
            renderInputThumbsRow(selectedNode());
...
async function runApiVideoGeneration(prompt, refs, runSettings=settings){
    ...
    try {
        ...
        const effUrl = ref => {
            // 手动绑定的 asset:// URI 优先（每张图片单独绑定，不需要开启可信素材开关）
            // 同时检查传入的 runSettings 和全局 settings，兼容不同调用路径
            const sourceUrl = mediaRefSourceUrl(ref);
            const manualAssetUri = ((runSettings.videoManualAssetUris || settings.videoManualAssetUris || {})[sourceUrl]) || '';
            if(manualAssetUri && /^asset:\/\//i.test(manualAssetUri)) return manualAssetUri;
            // 原有逻辑：素材库 asset_uris 自动匹配
        ...
        const hasManualAssetUris = refImages.some(img => /^asset:\/\//i.test(String(img.url || '')));
        const payload = {
            prompt,
            ...
            trusted_asset: useAssetUris || hasManualAssetUris
        };
