...
class LocalAssetClassificationSaveRequest(BaseModel):
    name: str = ""
...
@app.patch("/api/local-assets/classification")
async def save_local_asset_classification(payload: LocalAssetClassificationSaveRequest):
    filename, path = _local_upload_safe_path(payload.name)
    if not os.path.isfile(path):
        raise HTTPException(status_code=404, detail="文件不存在")
    kind, _ = _local_upload_kind_ext(filename, "")
    if kind != "image":
        raise HTTPException(status_code=400, detail="仅支持图片素材保存分类")
    classification = normalize_asset_classification(payload.classification or {})
    _write_local_upload_classification(filename, classification)
    item = _local_upload_item(filename)
    ...
    try:
        ...
        if "inputimagesensitivecontentdetected" in text.lower() or "privacyinformation" in text.lower() or "may contain real person" in text.lower():
            hint = (
                ...
                f"建议你这样处理：\n"
                f"  1. 如果该人物已在火山方舟注册为可信虚拟人像，请在智能画布的图片缩略图上点击盾牌图标🔗，输入 asset:// 认证地址；\n"
                f"  2. 改用非真人参考图，例如插画、AI 头像、商品图、场景图；\n"
